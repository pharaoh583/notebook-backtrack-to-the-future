...
void dft(CD* a, int n, bool inv){
	forsn(i,0,n)if(R[i]<i)swap(a[R[i]],a[i]);
	for(int m=2;m<=n;m*=2){
...
poly multiply(poly& p1, poly& p2){
	...
	int m=1,cnt=0;
	while(m<=n)m+=m,cnt++;
	forsn(i,0,m){R[i]=0;forsn(j,0,cnt)R[i]=(R[i]<<1)|((i>>j)&1);}
	forsn(i,0,m)cp1[i]=0,cp2[i]=0;
	forsn(i,0,p1.size())cp1[i]=p1[i];
	forsn(i,0,p2.size())cp2[i]=p2[i];
	dft(cp1,m,false);dft(cp2,m,false);
	forsn(i,0,m)cp1[i]=cp1[i]*cp2[i];
	dft(cp1,m,true);
	poly res;
	n-=2;
	forsn(i,0,n)res.pb((tf)floor(cp1[i].real()+0.5)); // FFT
